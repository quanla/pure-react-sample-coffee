...
const {IndexUtil} = require("./index-util");
...
exports.ArrayUtil = {
    isEqual(array1, array2, specificFields = []) {
        return array1 && array2 && array1.length == array2.length && array1.every(function(element, index) {
            if(typeof element === 'object') {
                let modifyObj = (obj) => _.pick(obj, specificFields);
                return _.isEqual(modifyObj(element), modifyObj(array2[index]))
            }
...
exports.openInNewWindow = (url, focus = false, width = 500, height = 620) => {
    const win = window.open(url, "", "menubar=no,width=" + width + ",height=" + height);
    focus && win.focus();
};
...
if (typeof window !== "undefined") {
    // TODO From Quan: this need removed
